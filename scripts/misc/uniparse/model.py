...
class Model:
	...
	def train(self, seed, train, dev_file, dev, epochs, batch_size, callbacks=None, patience=-1):
		"""
		Train neural model.
		...
		for epoch in range(1, epochs + 1):
			...
			if dev_file != None:
				### Edited part; added model 
				metrics = self.evaluate(dev_file, dev, seed, batch_size)
				###
	...
	def evaluate(self, test_file, test_data, seed, batch_size):
		"""
	...
	#		)
		...
		### Edited part
		directory = '/'.join(w for w in test_file.split('/')[ : -1])
		treebank = directory.split('/')[2]
		file_name = test_file.split('/')[-1].split('.')[0]
		pred_file = 'predict/' + treebank + '/' + file_name + '.uniparse' + '.' + seed
		print(pred_file)
		# write to file
		write_predictions_to_file(
			predictions,
		...
		metrics = evaluate_files(pred_file, test_file)
