	...
	for treebank in ['ewt', 'twitter', 'esl']:
		for seed in [1, 2, 3]:
			for emb in ['bert-base-cased', 'roberta-base', 'cardiffnlp-twitter-roberta-base']:
				if file_name + '.machamp.' + treebank + '.' + str(seed) + '.' + emb not in os.listdir('../predict/machamp/' + child + '/') or os.stat('../predict/machamp/' + child + '/' + file_name + '.machamp.' + treebank + '.' + str(seed) + '.' + emb).st_size == 0:
					os.system("python3 predict.py logs/machamp." + treebank + '.' + str(seed) + '.' + emb + '/*/model.tar.gz ../data/' + child + '/' + file + ' ../predict/machamp/' + child + '/' + file_name + '.machamp.' + treebank + '.' + str(seed) + '.' + emb + ' --device 0 --batch_size 16')
